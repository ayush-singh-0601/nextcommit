...
export function createMcpHandler(tools: Record<string, McpTool>): (request: McpRequest) => Promise<McpResponse> {
  return async (request) => {
    ...
    if (request.method === "tools/list") return mcpResult(request.id, { tools: Object.keys(tools).map((name) => ({ name })) });
    ...
    try { return mcpResult(request.id, { content: [{ type: "text", text: JSON.stringify(await tool((request.params?.arguments as Record<string, unknown>) ?? {})) }] }); }
    catch (error) { return mcpError(request.id, -32000, error instanceof Error ? error.message : "Tool failed"); }
  };
}
export async function startMcpServer(tools: Record<string, McpTool>, input = process.stdin, output = process.stdout): Promise<void> {
